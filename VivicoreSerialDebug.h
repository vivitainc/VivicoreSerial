/*
  VivicoreSerialDebug.h - Debug print library for VIVIWARE Cell Branch
...
#define DebugGPIODirectOut(DBG_GPIO_DDR, DBG_GPIO_BIT)                         \
  DBG_GPIO_DDR = DBG_GPIO_DDR | (1 << DBG_GPIO_BIT)
#define DebugGPIOLow(DBG_GPIO_PORT, DBG_GPIO_BIT)                              \
  DBG_GPIO_PORT = DBG_GPIO_PORT & ~(1 << DBG_GPIO_BIT)
...
#define Debug1ByteHexPrint(v)                                                  \
  ...
  } else { DBG_SERIAL_INSTANCE.print((uint8_t)(((v) >> 4) & 0x0F)); }            \
  if (((v) & 0x0F) > 9) {                                                        \
    DBG_SERIAL_INSTANCE.print((char)('A' + ((uint8_t)((v) & 0x0F) - 10)));       \
  } else { DBG_SERIAL_INSTANCE.print((uint8_t)((v) & 0x0F)); }

#define DebugHexPrint(v)                                                       \
  Debug1ByteHexPrint(v)
